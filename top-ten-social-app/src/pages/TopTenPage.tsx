...
function TopTenPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-2 text-gray-800">Top Ten</h1>
      <p className="text-gray-500 mb-6">
        Discover and share your top ten lists on any topic!
      </p>
      <form onSubmit={handleOnSubmit} className="flex-1 mb-6 gap-3">
      <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Enter a game title..."  className="flex-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />

      <button onClick={addGame} className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition">Add</button>
      </form>

      <ul className="space-y-3">
        {games.map((game) => (
          <li key={game.id} className="flex justify-between items-center bg-gray-50 p-3 rounded-lg border">
            <span className="text-gray-800">{game.title}</span>
            <button onClick={() => removeGame(game.id)} className="text-red-500 hover:text-red-700">X</button>
          </li>
        ))}
      </ul>
    </div>
